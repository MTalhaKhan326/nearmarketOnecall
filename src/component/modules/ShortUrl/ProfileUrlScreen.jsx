...
function ProfileUrlScreen() {
  const { userId } = useParams();
  const [isLoading, setIsLoading] = useState(true);
  const [path, setPath] = useState(null);
  const [error, setError] = useState(null);
  const API_BASE_URL =
    "https://rogvftzrsuaealt3f7htqchmfa0zfumz.lambda-url.eu-west-1.on.aws";

  useEffect(() => {
    setIsLoading(true);
    axios
      .get(`${API_BASE_URL}/short-urls/decode?id=${userId}`)
      .then(({ data }) => {
        if (data.status === 200 && data.data?.long_url) {
          const url = new URL(data.data.long_url);
          setPath(url.pathname + url.search);
          axios.post(`${API_BASE_URL}/short-urls/update-visit-count`, {
            id: data.data?.id,
          });
        } else {
...
export default ProfileUrlScreen;
